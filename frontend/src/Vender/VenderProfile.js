...
function VenderProfile() {
    ...
    return (
        ...
        <div className='h-95vh w-screen flex flex-row justify-center items-center'>
            <div className='rounded-3xl border-4 border-black' 
                style={{backgroundColor: `${user["color"]}`, height: "90%", width: "60%"}}>
                <div id="profileContainer" style={{backgroundColor: "#ffdb18"}}
                    className=' rounded-t-3xl relative h-1/4 border-b-2 border-black border-opacity-10 flex flex-row items-center'>
                    <img src={user["img"]} className="rounded-full border-4 border-black ml-6" 
                    style={{height:"100px", width:"100px", objectFit:"cover"}}></img>
                ...
                <div className='relative h-3/4 flex flex-row items-center p-5'>
                    <div style={{height: "95%"}}
                        ...
                        <div className='rounded-xl border-black border-4 p-5 m-2 mt-0 overflow-y-scroll scrollbar-thin relative' style={{height: "90%"}}>
                            {user["food"].map((mark, index) => (
                                <div className='flex flex-row items-center justify-between'>
                                    <div key={index} className="border-4 border-black my-4 p-2 overflow-x-clip rounded-3xl flex flex-row justify-between"
                                        style={{backgroundColor: "#FDF8E1", height: "50px", width: "80%"}}>
                                        <p className='font-bold'>{mark[0]}</p>
                                        <p>{mark[1]}</p>
                                    </div>
                                    <div className='border-4 text-2xl font-bold border-black rounded-3xl flex items-center justify-center'
                                        style={{height: "50px", width: "50px", cursor: "pointer", backgroundColor: "#FCEFB4"}}    
                                        onClick={(index) => {
                                            // Update server
                                        }}
                                    ><HiMinus /></div>
                        ...
                        <button className='rounded-xl border-black border-4 m-2' 
                    ...
                    <Map position={user["position"]} key={user["secretKey"]} />
                </div>
            </div>
        </div></>
    )
